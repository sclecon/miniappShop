...
class Painter extends BaseCurd
{
    ...
    /**
     * @ApiRouter(router="add", intro="新增画家", method="PUT")
    ...
    public function add(){
        ...
        $formData['avatar'] = Image::instance()->upload($this->request->file('image'), 'pointer');
    ...
    public function list(){
        ...
        if ($count){
            ...
            foreach ($list as $key => $value){
                $list[$key]['avatar'] = strpos($list[$key]['avatar'], 'http') === 0 ? $list[$key]['avatar'] : Http::instance()->getDomain().$list[$key]['avatar'];
            }
        }
